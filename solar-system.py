...
def orbits(dist, offset=0, clr='white', wdth=2):
    ...
    trace = go.Scatter3d(x=xcrd, y=ycrd, z=zcrd, marker=dict(
        size=0.1), line=dict(color=clr, width=wdth))
    return trace
...
distance_from_sun = [0, 57.9, 108.2, 149.6,
                     227.9, 778.6, 1433.5, 2872.5, 4495.1]
...
layout = go.Layout(title='Solar System', showlegend=False, margin=dict(l=0, r=0, t=0, b=0),
                   ...
                   scene=dict(xaxis=dict(title='Distance from the Sun',
                              ...
                              annotations=[
                       annot(
                           distance_from_sun[0], 40, 'Sun', xancr='left'),
